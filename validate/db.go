...
import (
	...
	"github.com/donbarrigon/forge/db"
...
func Unique(value reflect.Value, params ...string) (string, str.Placeholder, bool) {
	...
	col, e := db.Col(colName)
	if e != nil {
		ph.Append("error", e.Error())
		return "Error al validar unicidad del campo :field: :error", ph, true
	}
...
func Exists(value reflect.Value, params ...string) (string, str.Placeholder, bool) {
	...
	colName := params[0]
	field := params[1]
	ph := str.Placeholder{{Key: "collection", Value: colName}}
	...
	if e != nil {
		ph.Append("error", e.Error())
		return "Error al validar existencia del campo :field: :error", ph, true
	}
