...
import (
	...
	"github.com/donbarrigon/forge/handler"
...
type Validator interface {
	Rules() Rules
	PrepareForValidation(c *handler.Context) *errs.ValidationError
}

func Body(c *handler.Context, v Validator) *errs.Error {
	if e := c.UnmarshalBody(v); e != nil {
		return e
	}

	ve := v.PrepareForValidation(c)
	if ve == nil {
		ve = errs.NewValidationError()
	}

	rules := v.Rules()
	return validate(c, v, rules, ve)
}

func validate(c *handler.Context, validator any, rules Rules, e *errs.ValidationError) *errs.Error {
	...
	if val.Kind() != reflect.Struct {
		return errs.InternalMsg("Error de validación: Los datos tienen que ser un *struct", nil)
	}
	typ := val.Type()
