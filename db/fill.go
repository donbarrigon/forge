...
// @return error
func Fill(model any, request any) *errs.Error {

	modelValue := reflect.ValueOf(model)
	requestValue := reflect.ValueOf(request)

	if modelValue.Kind() != reflect.Ptr || requestValue.Kind() != reflect.Ptr {
		return errs.InternalMsg("The model and the request must be pointers.", nil)
	}
	...
	if modelValue.Kind() != reflect.Struct || requestValue.Kind() != reflect.Struct {
		return errs.InternalMsg("The model and the request must be structs.", nil)
	}
...
// @return error
func Filld(model OdmModel, request any) *errs.Error {
